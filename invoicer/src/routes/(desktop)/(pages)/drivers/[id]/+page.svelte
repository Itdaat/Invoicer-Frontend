<script>
	...
	import { getFullPerson } from '$lib/api/server/persons';
	...
	$: personApi = getPersonFormatted(id);
	$: if (updated) {
		updated = false;
		personApi = getPersonFormatted();
...
{:then person}
	...
	<div class="container" in:fade={{ duration: 300, delay: 200 }} out:fade={{ duration: 200 }}>
		<div class="header">
			<div class="title">{person?.nickname}</div>
			<div class="right">
		...
		<div class="main">
			<div class="main-container">
				<MiniCategory title={t.person_main}>
					<ListItem name={t.person_create_first_name} value={person?.firstName} messageText={t.person_copied_first_name} />
					<ListItem name={t.person_create_last_name} value={person?.lastName} messageText={t.person_copied_last_name} />
				</MiniCategory>
